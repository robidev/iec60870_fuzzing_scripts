...
def main():
    ...
    session = Session(
        target=Target(
            connection=SocketConnection("10.84.134.11", 2404, proto='tcp'),
    ...
    s_byte(0x68, fuzzable=False ) # start
    s_byte(0x04, fuzzable=False) # len
    s_byte(0x07, fuzzable=False) # c1 lsb
    s_byte(0x00, fuzzable=False) #    msb
    s_byte(0x00, fuzzable=False) # c2 lsb
    s_byte(0x00, fuzzable=False) #    msb
    ...
    s_initialize("GI")
    s_byte(0x68, fuzzable=False) # start
    s_byte(0x0e, fuzzable=False) # len
    s_byte(0x00, fuzzable=False) # c1
    s_byte(0x00, fuzzable=False) #
    s_byte(0x02, fuzzable=False) # c2
    s_byte(0x00, fuzzable=False) #

    s_byte(0x64, fuzzable=False) # type
